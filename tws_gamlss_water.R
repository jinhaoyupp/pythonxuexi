...
tws.gamlss.weibull = gamlss(y~cs(x)+cs(water), data = tws, family = WEI)
tws.gamlss.gamma = gamlss(y~cs(x)+cs(water), data = tws, family = GA)
tws.gamlss.logno = gamlss(y~cs(x)+cs(water), data = tws, family = LOGNO)
tws.gamlss.logistic = gamlss(y~cs(x)+cs(water), data = tws, family = LO)
tws.gamlss.gumbel = gamlss(y~cs(x)+cs(water), data = tws, family = GU)
...
# mu固定不变
tws.gamlss.logistic.fix = gamlss(y~1, data = tws, family = LO)
plot(setaxis, tws$y, type = 'l',col=1)
lines(setaxis, fitted(tws.gamlss.logistic.fix),col=2)

# 分位数
centiles(tws.gamlss.logistic.fix,setaxis,cent = c(5,95))
lines(setaxis, tws$y, type = 'l',col=1)
lines(setaxis, fitted(tws.gamlss.logistic.fix), col='red')
